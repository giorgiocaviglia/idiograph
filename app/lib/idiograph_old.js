!function(){
	...
	d3.graph = function(){
        ...
        // force directed graph
		      ...
		      .call(d3.svg.brush()
				...
				drag = drag || force.drag()
			 	...
			 	nodeTip = d3.tip().attr("class","d3-tip").html(function(d) { console.log(d); return d.data.name; });
			...
			function keydown(){
			    ...
			    case 99: // delete
			      nodes.filter(function(d){ return d.selected; })
			      	.forEach(function(d){ d.hide = true; })
			      update();
			      break;
			  }
        ...
        var nodeElements = n.filter(function(d){
        		return !d.hide;
        	})
	        .map(function (d,i) {
	        	console.log(d.id)
	        	d.id = d._id //? d.id : i + 1; // create an id only the first time
	        	d.type = 'node';
		...
		chart.editing = function(_) {
			...
			editing = _;
			if(svg) svg.classed("editing", editing)
			return chart;
		};
